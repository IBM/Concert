...
### Control Name
**MSP-Controlled Encryption Keys**

### Control Synopsis
Encryption keys SHALL be owned and controlled by the MSP and access to key to the respective role access.

### Control Description
The platform shall ensure encryption keys used for protecting data are generated, stored, and
managed within the sovereign environment. Access to encryption keys shall be restricted to authorized MSP roles and shall not be accessible to
external providers or vendors. Key management systems shall enforce role-based access controls and audit logging for all key
operations.
