...
UPLOAD_CERTS_COMMAND="upload-concert --upload-config /toolkit-data/certs-config.yaml"
echo "${CONTAINER_COMMAND} ${OPTIONS} -v ${outputdir}:/toolkit-data ${CONCERT_TOOLKIT_IMAGE} bash -c ${UPLOAD_CERTS_COMMAND}"
${CONTAINER_COMMAND} ${OPTIONS} -v ${outputdir}:/toolkit-data ${CONCERT_TOOLKIT_IMAGE} bash -c "${UPLOAD_CERTS_COMMAND}"