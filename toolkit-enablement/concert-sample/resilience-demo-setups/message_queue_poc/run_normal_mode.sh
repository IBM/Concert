...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Starting NORMAL Mode Simulator${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Start simulator
echo -e "${GREEN}Starting normal mode simulator...${NC}"
python unified_threshold_simulator.py --mode normal --queue threshold_all_metrics &
SIMULATOR_PID=$!

echo ""
echo -e "${GREEN}✓ Simulator started (PID: ${SIMULATOR_PID})${NC}"
echo ""
echo -e "${YELLOW}Waiting 15 seconds for metrics to stabilize...${NC}"
sleep 15
